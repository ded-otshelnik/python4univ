...
7. **Калькулятор**. Напишите код, который получает на входе строку, представляющую собой некоторое математическое выражение (только '+', '-', '*', '/') с целыми числами в качестве операндов (например, "3+3\*3/3"). Использовать функцию eval() не допускается.
