...
Реализовать простое веб-приложение, используя принципы MVC и RESTful API, по вариантам.

### Требования

1. Для веб-приложения необходимо использовать фреймворк Flask, Django или FastAPI.
2. Пользователь должен взаимодействовать с приложением либо через отдельную программу-клиент (консольную, для простоты), или сервис для отправки запросов к API (Postman, например).
...
6. Реализовать минимум 3 теста для проверки API. Тесты должны запускаться через `tox`.
